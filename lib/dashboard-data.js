...
// AwaOwn products carry one option dimension ("Colour" / "Size" / "Variety").
// `product.variants` is the flat list of varieties, each with its own price,
// stock and image. `selectedId` is a ProductVariant id.
const variantOptions = (v) => v?.options ?? v?.optionValues ?? {};

/** Does this combination row match the shopper's per-axis selection? */
function comboMatchesSelection(v, selection) {
  const ov = variantOptions(v);
  const keys = Object.keys(ov);
  if (!keys.length) return false;
  return keys.every((k) => selection?.[k] === ov[k]);
}

/**
 * Resolve the price / stock / image / label for what the shopper has picked.
 * `selection` is a per-axis map, e.g. { colour: "midnight", "ssd-size": "256-gb" }.
 */
export function resolveVariant(product, selection) {
  const list = product?.variants ?? [];
  if (!product?.hasVariants || list.length === 0) {
    return {
      variantId: null,
      variantLabel: null,
      ...
      maxQty: product?.stock == null ? 99 : Math.max(0, product.stock),
      complete: true,
    };
  }
  const axes = product.variantAxes ?? [];
  const exact = list.find((v) => comboMatchesSelection(v, selection));
  const v = exact || list.find((x) => x.inStock) || list[0];
  const ov = variantOptions(v);

  const label = axes.length
    ? axes
        .map((ax) => {
          const opt = ax.options?.find((o) => o.value === ov[ax.key]);
          return opt ? `${ax.name}: ${opt.label}` : null;
        })
        .filter(Boolean)
        .join(" · ")
    : `${product.optionName ?? "Option"}: ${v.label}`;
  ...
  return {
    ...
    price: v.price,
    inStock: v.inStock,
    maxQty: v.stock == null ? 99 : Math.max(0, v.stock),
    // every axis has a chosen value that resolves to a real combination
    complete: !!exact,
  };
}

/** First in-stock combination's per-axis map, so the picker starts on something valid. */
export function defaultVariantSelection(product) {
  const list = product?.variants ?? [];
  const first = list.find((v) => v.inStock) || list[0];
  return first ? { ...variantOptions(first) } : {};
}
...
export const paymentMethods = [
  {
    ...
    description: "Visa, Mastercard, Verve. Secured by Paystack",
  },
  {
    id: "wallet",
    label: "AwaOwn Wallet",
