...
export const ordersApi = baseApi.injectEndpoints({
  endpoints: (build) => ({
    ...
    // Live coupon check for a partner store's checkout - same "Apply" button
    // as the main checkout, but priced from the item list since there's no
    // server-side cart to read yet in a guest flow.
    guestPreviewCoupon: build.mutation({
      query: (body) => ({ url: "/orders/guest/coupon/preview", method: "POST", body }),
    }),
