...
export const merchantApi = baseApi.injectEndpoints({
  endpoints: (build) => ({
    ...
    setMerchantDiscount: build.mutation({
      query: ({ productId, discountAmount, discountType, discountPercent }) => ({
        url: `/merchant/products/${productId}/discount`,
        method: "PATCH",
        body: { discountAmount, discountType, discountPercent },
      }),
      invalidatesTags: (result) => [
        "MerchantProduct",
        { type: "Product", id: "LIST" },
    ...
    acceptMerchantTerms: build.mutation({
      query: () => ({ url: "/merchant/accept-terms", method: "POST" }),
    }),
...
export const {
  ...
  useAcceptMerchantTermsMutation,
