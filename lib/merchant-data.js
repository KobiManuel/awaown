...
// AwaOwn only operates in Lagos and Abuja for now. Every address / location
// picker on the app uses this list; widen it here when new cities open up.
export const NIGERIAN_STATES = ["Lagos", "Abuja"];

export const SERVICE_AREA_NOTE =
  "AwaOwn currently operates in Lagos and Abuja only. More cities coming soon.";

// City/LGA options per state. City pickers must always be dependent on (and
// disabled until) a state is chosen. Keys must match NIGERIAN_STATES exactly.
...
// AwaOwn keeps this share of a partner's profit as a platform fee. Kept in sync
// with the backend (orders.service.ts PLATFORM_PARTNER_FEE_RATE).
export const PARTNER_PLATFORM_FEE_RATE = 0.2;

/**
 * Suggested partner payout for a given customer price. The ₦1,000 floor always
 * applies; this is just guidance so vendors know what competitive payouts look
 * like. Percentage bands taper as the price rises, rounded to the nearest ₦500.
 *
 *   ₦20,000  → ₦2,000 – ₦3,000
 *   ₦50,000  → ₦4,000 – ₦5,500
 *   ₦100,000 → ₦5,000 – ₦7,000
 */
export function partnerPayoutSuggestion(price) {
  const p = Number(price) || 0;
  if (p <= 0) return null;
  let lowPct;
  let highPct;
  if (p <= 20000) [lowPct, highPct] = [0.1, 0.15];
  else if (p <= 50000) [lowPct, highPct] = [0.08, 0.11];
  else if (p <= 100000) [lowPct, highPct] = [0.05, 0.07];
  else if (p <= 250000) [lowPct, highPct] = [0.035, 0.05];
  else [lowPct, highPct] = [0.025, 0.04];
  const round500 = (n) =>
    Math.max(PARTNER_PROGRAM_MIN_PROFIT, Math.round(n / 500) * 500);
  return { low: round500(p * lowPct), high: round500(p * highPct) };
}
