...
const partnerSlice = createSlice({
  ...
  reducers: {
    hydratePartner(state, action) {
      ...
      if (payload.ownProducts) state.ownProducts = payload.ownProducts;
    ...
    addOwnProduct(state, action) {
      state.ownProducts.unshift(action.payload);
    },
    removeOwnProduct(state, action) {
      state.ownProducts = state.ownProducts.filter((p) => p.id !== action.payload);
    },
    toggleOwnProductField(state, action) {
      const { id, field } = action.payload;
      const product = state.ownProducts.find((p) => p.id === id);
