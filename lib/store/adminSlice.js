...
const initialState = {
  ...
  sectionVisibility: sectionVisibilityDefaults,
  faqs: faqsSeed,
};
...
const adminSlice = createSlice({
  ...
  reducers: {
    hydrateAdmin(state, action) {
      ...
      if (payload.sectionVisibility) state.sectionVisibility = payload.sectionVisibility;
    ...
    reopenComplaint(state, action) {
      const complaint = state.complaints.find((c) => c.id === action.payload);
      if (complaint) {
        complaint.status = "open";
        logAction(state, `Reopened complaint from ${complaint.customer} (${complaint.order})`);
      }
    },
    ...
    saveSectionVisibility(state, action) {
      state.sectionVisibility = action.payload;
      logAction(state, "Updated homepage section visibility");
    },
    addFaq(state, action) {
      state.faqs.unshift(action.payload);
      logAction(state, `Added FAQ "${action.payload.question}"`);
    },
    ...
    addTeamMember(state, action) {
      ...
      logAction(state, `Added ${action.payload.name} (${action.payload.email}) as ${action.payload.role}`);
    },
    removeTeamMember(state, action) {
      const member = state.team.find((t) => t.id === action.payload);
      state.team = state.team.filter((t) => t.id !== action.payload);
      if (member) logAction(state, `Removed ${member.name} from the team`);
    },
